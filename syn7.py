...
[수행 규칙]
1. 각 문항을 순서대로 빠짐없이 응답하세요.
2. 분기(※ 분기) 표시가 명시된 문항만 해당 조건에 따라 건너뛰고 "SKIP"으로 표시하세요.
   분기 표시가 없는 문항은 어떤 경우에도 반드시 응답하세요. SKIP 금지.
3. 문항 내용이 이전 문항과 유사하거나 중복되어도 각각 독립적으로 응답하세요.
   중복이라는 이유로 SKIP하지 마세요.
4. 문맥상 무의미하다고 판단해도 임의로 SKIP하지 마세요. 반드시 보기 중 하나를 선택하세요.
5. 선택형(single/multi)은 보기 번호(숫자)만 응답하세요.
6. 자유응답(open)은 당신의 프로필에 맞는 짧고 구체적인 한 문장으로 응답하세요.
7. 출력 형식을 반드시 준수하세요. 다른 설명 금지.
